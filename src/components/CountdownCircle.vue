<template>
  <div class="countdown__timer">
    <h1 class="countdown__number">
      {{ countdownNumber
      }}<span class="countdown__time-format">{{ timeFormat }}</span>
    </h1>
    <svg class="countdown__circle">
      <circle class="countdown__inside-circle" r="45" cx="75" cy="75"></circle>
      <circle
        ref="outsideCircle"
        class="countdown__outside-circle"
        r="60"
        cx="75"
        cy="75"
      ></circle>
...
export default {
  ...
  mounted() {
    ...
    switch (this.timeFormat) {
      case 'days':
        this.$refs.outsideCircle.style.strokeDashoffset =
          (1 - this.countdownNumber / this.period_days) * PERIMETER
        break
      case 'hrs':
        this.$refs.outsideCircle.style.strokeDashoffset =
          (1 - this.countdownNumber / 24) * PERIMETER
        break
      case 'min':
        this.$refs.outsideCircle.style.strokeDashoffset =
          (1 - this.countdownNumber / 60) * PERIMETER
        break
      case 'sec':
  ...
  watch: {
    countdownNumber: function () {
      switch (this.timeFormat) {
        case 'days':
          this.$refs.outsideCircle.style.strokeDashoffset =
            (1 - this.countdownNumber / 10) * PERIMETER
          break
        case 'hrs':
          this.$refs.outsideCircle.style.strokeDashoffset =
            (1 - this.countdownNumber / 24) * PERIMETER
