...
export default {
  ...
  mounted() {
    const items = document.querySelectorAll('.roadmap__content, .hide-down')

    const callbackFunc = () => {
      for (let i = 0; i < items.length; i++) {
        if (this.isElementInView(items[i])) items[i].classList.add('is-in-view')
        else items[i].classList.remove('is-in-view')
      }
    }
